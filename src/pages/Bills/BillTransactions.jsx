...
import { EllipsisVertical, Eye, X, Receipt } from 'lucide-react'
import {
    Dialog,
    DialogContent,
    DialogHeader,
...
export default function BillTransactions() {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['bill-transactions', id, currentPage, rowsPerPage],
        queryFn: async () => {
            const response = await api.get(`/sections/bills/${id}/transactions`, {
                params: {
                    page: currentPage,
    ...
    const { data: billData } = useQuery({
        ...
        queryFn: async () => {
            const response = await api.get(`/sections/bills/${id}`)
            return response.data
        }
    })
        ...
        >
            <div className="flex flex-col h-full gap-4">
                <PageHeading
                    ...
                    description="View and manage bill transactions"
                />
                ...
                <div className="flex-1">
                                    ...
                                    <div>
                                        <div className="flex items-center space-x-2 mb-0.5">
                                            <span className="text-sm font-medium text-foreground">
                                                {billData.data.attributes.name}
                                            </span>
                                        </div>
                                        <p className="text-xs text-muted-foreground">
                                            Code: {billData.data.attributes.code}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center space-x-6">
                                    <div className="text-right">
                                        ...
                                        <p className="text-sm font-medium text-foreground">
                                            {billData.data.relationships?.bill?.attributes?.number}
                                        </p>
                                    </div>

                                    <div className="text-right">
                                        <p className="text-xs text-muted-foreground">Biller Name</p>
                                        <p className="text-sm font-medium text-foreground">
                                            {billData.data.relationships?.bill?.attributes?.billerName}
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    {transactions.length === 0 ? (
                        <EmptyComponent
                            ...
                            description="This bill doesn't have any transactions yet."
                                ...
                                <>
                                    ...
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                {billData.data.relationships?.bill?.attributes?.number} - {billData.data.relationships?.bill?.attributes?.billerName}
                                            </p>
                                        </div>
                                    </div>
